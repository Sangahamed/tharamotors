...
<div class="bg-white border-b border-slate-100 sticky top-[72px] z-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <nav class="flex items-center gap-2 text-xs sm:text-sm overflow-x-auto whitespace-nowrap pb-1">
            <a href="/" class="text-slate-500 hover:text-accent-500 transition">Accueil</a>
            ...
            <a href="{{ route('vehicules.occasion') }}" class="text-slate-500 hover:text-accent-500 transition">Véhicules d'occasion</a>
            <svg class="w-3 h-3 sm:w-4 sm:h-4 text-slate-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <span class="text-slate-900 font-medium truncate">{{ $vehicle->year }} - {{ $vehicle->brand }} {{ $vehicle->model }}</span>
        </nav>
    </div>
</div>
...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div class="space-y-4">
            <!-- Image principale -->
            <div class="relative rounded-2xl overflow-hidden bg-slate-100 aspect-[4/3] shadow-lg group">
            ...
            <div class="grid grid-cols-4 gap-2 sm:gap-3">
                ...
                @forelse($images as $index => $image)
                    <button onclick="changeImage('{{ asset('storage/vehicles/' . $image) }}', this)"
                            class="rounded-lg overflow-hidden border-2 {{ $index === 0 ? 'border-accent-500' : 'border-transparent hover:border-accent-300' }} aspect-square transition-all">
        ...
        <div class="space-y-6">
            <!-- Titre et prix -->
            <div>
                <p class="text-sm text-slate-500 mb-1">Réf: {{ $vehicle->id }}</p>
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold mb-2 text-primary-700">
                    {{ $vehicle->year }} - {{ $vehicle->brand }} {{ $vehicle->model }}
                </h1>
                <div class="flex items-baseline flex-wrap gap-3 mb-4">
                    <span class="text-3xl sm:text-4xl font-extrabold text-accent-800">{{ number_format($vehicle->price, 0, '', ' ') }} FCFA</span>
                    @if($vehicle->old_price ?? false)
                        <span class="text-base sm:text-lg text-slate-400 line-through">{{ number_format($vehicle->old_price, 0, '', ' ') }} FCFA</span>
                    @endif
                </div>

                <!-- Tags (carburant, transmission, kilométrage) -->
                <div class="flex flex-wrap gap-3 mb-6">
                    <div class="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-white bg-primary-600">
                    ...
                    <div class="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium bg-slate-200 text-slate-700">
                         <span class="w-5 h-5 inline-block">
            ...
            <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
                <h3 class="text-lg font-bold mb-3 text-primary-700">Description</h3>
            ...
            <!-- Caractéristiques -->
            <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
                <h3 class="text-lg font-bold mb-4 text-primary-700">Caractéristiques</h3>
                    ...
                    @foreach([
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-lg flex items-center justify-center bg-primary-50">
                                ...
                                @elseif($spec['icon'] === 'speedometer')
                                    <span class="w-5 h-5 inline-block text-primary-600">
                        ...
                        <img src="{{ asset('images/Carburant.svg') }}" alt="Carburant" class="w-full h-full object-contain">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-slate-900">{{ $spec['value'] ?: 'Non renseigné' }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <a href="tel:+2250565355079"
                       class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 font-semibold transition-all hover:bg-slate-50 border-primary-600 text-primary-600">
                    ...
                    <button onclick="shareVehicle('{{ route('details', $vehicle) }}', '{{ addslashes($vehicle->brand . ' ' . $vehicle->model . ' ' . $vehicle->year) }}')"
                            class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-slate-300 text-slate-700 font-semibold transition-all hover:bg-slate-50">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if(isset($similarVehicles) && count($similarVehicles) > 0)
        <section class="mt-16">
            <h2 class="text-2xl font-extrabold mb-8 text-primary-700">Véhicules similaires</h2>
                                ...
                                <div class="pt-2 pb-4 px-4 bg-primary-600">
                                    <div class="flex items-end justify-between">
                                        <div>
                                            ...
                                            <p class="text-xl font-extrabold text-accent-400">{{ number_format($similar->price, 0, '', ' ') }} FCFA</p>
                                        </div>
                                        <div class="flex flex-col gap-1">
                                            <div class="flex items-center gap-1.5 px-2 py-0.5 rounded text-xs text-white bg-primary-800">
...
<script>
    function changeImage(src, element) {
        ...
        document.querySelectorAll('.grid-cols-4 button').forEach(btn => {
            btn.classList.remove('border-accent-500');
            btn.classList.add('border-transparent', 'hover:border-accent-300');
        });
        element.classList.remove('border-transparent', 'hover:border-accent-300');
        element.classList.add('border-accent-500');
    }
</script>
@endpush